<script lang="ts">
  ...
  import { StoreName, fetchSettings, updateInDatabase } from '../../shared/functions/Database';
...
<div id="workout" class="flex flex-col items-center">
  ...
  <div class="exercice-container collapse collapse-arrow bg-base-100 my-2 w-5/6 override-collapse w-full">
    ...
    <input type="checkbox" name="exercice" checked={e.isSelfOpen} class="cursor-pointer" on:click={() => {onOpenExercice(e)}}/>
    ...
    <div class="collapse-title text-xl font-medium text-primary w-full mx-2 override-collapse-title">
      ...
      <span class="text-secondary text-sm">{`${e.slGUI.length} Sets`}</span>
      {#if e.getMaxWeight(settings.wm)}
        <span class="text-secondary text-sm">{` - Max : ${e.getMaxWeight(settings.wm)}${getReducedStringMetric(settings.wm)}`} {e.isSelfOpen} {e.isExtraOpen}</span>
      {/if}
    </div>
    ...
    {#if e.isSelfOpen}
       ...
       <div class="collapse-content">
         ...
         <ExerciceForm e={e} on:update={(event) => {
          workoutGUI.el = [];
          for (const eGUI of workoutGUI.elGUI) workoutGUI.el.push(eGUI.convertToExercice());
          updateWorkout();
        }}/>
   
         <!-- svelte-ignore a11y-click-events-have-key-events -->
         <div class="w-full flex justify-end cursor-pointer" on:click={() => e.isExtraOpen = !e.isExtraOpen}>
          {#if e.isExtraOpen}
          <Icon icon={minusIcon} color="blue" width="15" height="15"/>
          {:else}
          <Icon icon={plusIcon} color="blue" width="15" height="15"/>
          {/if}
         </div>
       </div>
    {/if}
